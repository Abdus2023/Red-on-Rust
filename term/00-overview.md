...
| `01-dictionary.md` | the 78 canonical terms, with all seven required fields | generated |
...
All seven fields are present on all 78 terms. All nine distinctions are enforced as laws (§4) and, where the source itself conflates them, filed as collisions (§5). 75 collisions are reported, including 4 that block work.
...
5. **Bidirectional linking.** `Term.collisions` and `Collision.affects` are checked against each other in both directions; 301 links, zero asymmetries, zero dangling `T-`/`N-`/`X-` references.
...
75 collisions: **4 BLOCKING**, 51 MAJOR, 19 MINOR, 1 INFO. 72 are new findings; 3 extend or correct an existing `C-`/`U-`/`AMB-` entry and say so. X-64…X-68 come from a fault-taxonomy audit that re-read every `Fault`-family declaration, and X-69…X-75 from a **declaration sweep** that checked every `Enum::Variant` path in L1–42312 against every declaration of that enum — 36 findings across nine enums, of which twelve are undeclared `Fault::` paths (X-69) and eight are undeclared `MachineEvent::` paths (X-71). That sweep also **retracted a claim the fault audit had itself made**: X-64 was first filed as a `PHANTOM-IDENTIFIER` on the assertion that “`Fault::StalePlan` occurs nowhere in L1–42312”, four documents were amended on that premise, and the premise is false — the string occurs verbatim at L28373. X-64 is refiled as `UNDECLARED-VARIANT`, the four amendments are reverted with the withdrawn wording quoted, and the retraction is recorded in §6 rather than silently undone. The same audit had earlier found and corrected a false claim this pass wrote into `spec/06` C-08.

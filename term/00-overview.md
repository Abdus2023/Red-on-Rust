...
| `01-dictionary.md` | the 74 canonical terms, with all seven required fields | generated |
| `02-collisions.md` | the 68-entry collision register, each cited to a frozen line | generated |
...
All seven fields are present on all 74 terms. All nine distinctions are enforced as laws (§4) and, where the source itself conflates them, filed as collisions (§5). 63 collisions are reported, including 3 that block work.
...
The other 48 terms are not required by name but are load-bearing: the ones the required terms are defined in terms of (`EffectDigest` T-21, `Constraint` T-12, `CapabilityKernel` T-13, `CapabilityError` T-72), the ones a required distinction needs (`LLMOutput` T-56, `ReferenceModel` T-58, `Implementation` T-65, `Verification` T-66, `Proof` T-67, `EvidenceStatus` T-68, `Frozen` T-69), and the ones the collisions turn on (`Value` T-31, `Expr` T-30, `Frame` T-33, `GlobalState` T-38, `WalRecord` T-47, `CanonicalEnvelope` T-62, `CanonicalPayload` T-63, and the five fault-taxonomy types added by the last audit — `Fault` T-70, `GlobalFault` T-71, `MarshalFault` T-73, `HostFault` T-74).
...
1. **Attestation before definition.** Every term was located in `Red-on-Rust.md` by case-sensitive search, then read in context at each declaration site. No `FIRST_DEFINITION` is a raw first grep hit: each was confirmed by reading the line. `term/_check.py` re-greps all 677 citations on every run and fails if a quoted token is not on the line it is cited to.
...
5. **Bidirectional linking.** `Term.collisions` and `Collision.affects` are checked against each other in both directions; 263 links, zero asymmetries, zero dangling `T-`/`N-`/`X-` references.
...
68 collisions: **4 BLOCKING**, 45 MAJOR, 18 MINOR, 1 INFO. 65 are new findings; 3 extend or correct an existing `C-`/`U-`/`AMB-` entry and say so. The five added last (X-64…X-68) come from a fault-taxonomy audit that re-read every `Fault`-family declaration; that audit also found and corrected a false claim this pass had itself written into `spec/06` C-08 (§6).
...
The register's thirteen kinds are: `TYPE-HOMONYM`, `TYPE-ROLE-HOMONYM`, `FIELD-SET`, `SYMBOL-OVERLOAD`, `PREDICATE-SIGNATURE`, `STAGE-ORDER`, `STATE-VS-RECORD`, `UNDEFINED-TYPE`, `METHOD-SIGNATURE`, `ALIAS-DEFECT`, `SCOPE-DRIFT`, `PROVENANCE-DEFECT`, `PHANTOM-IDENTIFIER`.
...
| `req/03-ambiguous.md` | AMB-24's README citations corrected to L12 and L732 — they were first corrected to L708, which this pass then invalidated by adding the README's `term/` paragraph, so the block is now cited by its `# Project Status` heading as well as by line — with the defective ranges kept; AMB-25's phantom `PlannerState` struck and quoted; **AMB-08 corrected in place** (the phantom ninth variant `StalePlan` struck from the frozen list and quoted; the v0.3 name citation moved from L8748–8757 to L8758; the `IsolationBreach` → “a `Capability` reason” mapping withdrawn; the evidence range L23806–23824 narrowed to L23806–23816); **AMB-30, AMB-31, AMB-32** added, then **AMB-33 (blocking), AMB-34, AMB-35** added by the fault-taxonomy audit; §3 counts updated 28 → 31 → **34** ambiguities and 45/16 → 53/19 → **58**/19 cross-references | X-59, X-62…X-68 |
| `req/_validate.py` | the expected register sizes updated **45 → 53 → 58** `C-` rows and **16 → 19** `U-` headings, with a comment naming the pass and the audit that grew them, so the change is recorded and not silent drift. This checker also caught the pass introducing three identifiers (`Revoked`, `Expired`, `InvalidConstraint`) into REQ-CALC-013's INVARIANTS that were outside its cited ranges; the record's SOURCE line was extended to L20408–20413, L20451 and L20835 rather than the tokens being dropped | X-59, X-66 |
| `README.md` | L29 and L32 counts and the `term/` paragraph (L61–L83) updated (57 findings in 58 rows; 19 open decisions; 677 citations re-grepped) and a `term/` paragraph added after the `dep/` paragraph describing the four generated files, the nine enforced distinctions and the standing verifier | X-52, X-53, X-63 |
| `spec/01-canonical-specification.md L138 (R-CALC-06)` | a **non-normative annotation** appended to the same line: `StalePlan` is in none of the seven `pub enum Fault` declarations and `Fault::StalePlan` occurs nowhere, so the parenthetical “plus `StalePlan` at the planner boundary” is a canonicalization-layer addition; the frozen set is not closed (L23807 is an explicit `// ... (previous faults)` elision); `HostFault` in `Host(HostFault)` is the type with the eight-undeclared-variant gap. **The normative sentence is unchanged and still governs** | X-64, X-67, X-68 |
...
| `req/01-registry-part2-semantics.md L181, L186, L197, L199` | REQ-CALC-013's INVARIANTS corrected in place (“closed set; inner variants not enumerated” is false on both halves) and its SOURCE extended to the `CapabilityError` declaration and both contradictory use sites; REQ-CALC-014's STATEMENT corrected (`StalePlan` is not a taxonomy member) and its POSTCONDITIONS `Fault::StalePlan` **withdrawn** with the postcondition the source does support substituted | X-59, X-64, X-66 |

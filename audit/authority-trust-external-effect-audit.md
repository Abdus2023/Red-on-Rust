...
2. **SEC-001, SEC-002 — ADDENDUM DRAFTED, pending adoption**: the freeze exists as a proven draft — `audit/sec001-sec002-addendum-draft.md` (generated from `audit/spec_addendum.py`'s constants, so texts cannot drift; dry-run proof: 152/152/148 layers parse, D1=0, zero warnings on the four new obligations, D3 overlaps 0.78–0.92, sandboxed index rebuild 152/76/21/18). It freezes R-EFFECT-08 (receipt-result admission), R-KERN-04 (holder-possession binding; resolves the newly registered C-77 conflict), R-KERN-05 (CapabilityContext real type), R-COMPILE-06 (plan-bound capability literals, U-22 security-direction closure), mutations M019–M021, and tag `EFFECT-RECEIPT-RESULT-NO-AUTHORITY`. Until adopted, the central thesis `LLMOutput ∧ UntrustedInput ↛ ExternalEffect` remains falsifiable by a conforming implementation, and every other guarantee is derivative.

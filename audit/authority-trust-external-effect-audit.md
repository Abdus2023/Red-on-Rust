...
1. **SEC-023 — RESTORED (this branch)**: the 48 substituted `spec/01` bodies and the 48 records' Normalized fields were restored in place via `audit/spec_restore_dryrun.py --apply` (restoration texts = each record's own Original, verified against `req/`; substituted wording retained as "Superseded Normalized (SEC-023 substitution)" quotes per R-SCOPE-03; review record: `audit/spec01-restoration-draft.md`). Post-apply verification: **`spec/_check.py` D1 = 0 (gate promoted, exit 0; `--strict` available)**, `req/_validate.py` 0 errors / 148-of-148 obligations, `spec/_build_index.py` regenerates `spec/10-index.json` byte-identical. The remaining D2/D3 items are adjudicated terse-body/label warnings on D1-clean obligations (owner may re-word matrix rows and adopt `--strict`). *Until this restoration, every other remediation was undraftable: the frozen addenda this audit prescribes would have been written against a layer that no longer contained the rules they amend.*
2. **SEC-001, SEC-002 — FROZEN (addendum adopted, this branch)**: the addendum was applied via `audit/spec_addendum.py --apply` — R-EFFECT-08 (receipt-result admission), R-KERN-04 (holder-possession binding; resolves the newly registered C-77 conflict), R-KERN-05 (CapabilityContext real type), R-COMPILE-06 (plan-bound capability literals, U-22 security-direction closure), mutations M019–M021, tag `EFFECT-RECEIPT-RESULT-NO-AUTHORITY`, with the matrix/records/README companions and the index builder extended so regeneration cannot drop it. Post-apply: 152 obligations / 152 matrix rows / 148 records, `spec/_check.py` D1=0 exit 0 (only the pre-existing adjudicated warnings), `req/_validate.py` 0 errors, index rebuilt at 152/76/21/18. Review record: `audit/sec001-sec002-addendum-draft.md`. The central thesis `LLMOutput ∧ UntrustedInput ↛ ExternalEffect` is no longer falsifiable *at the specification level* by these two channels; the remaining exposure is implementation-level (no implementation exists yet — every obligation is SPECIFIED).
3. **SEC-016, SEC-018 — FROZEN (addendum II adopted, this branch)** (`audit/spec-addendum2-draft.md` review record; `audit/spec_addendum2.py --apply`): **R-CORE-11** freezes one signature set for the central theorem's predicates (`ValidatedRequest(E)` request-time subsuming `ValidatedPlan`; `Authorized(holder, c, E, t)` with possession as a conjunct, formalizing R-KERN-04; the other five `Authorized` signatures superseded; resolves C-80 and term/ X-01/X-04/X-05 at the normative layer), and **R-MARSHAL-06** defines `contains_capability` as a frozen total predicate whose closed traversal domain includes closure environments (`FunctionValue.env`, recursively; resolves C-81, mutation M032). An ill-formed invariant and an undefined boundary predicate make all other remediations unadjudicable.
4. **SEC-003, SEC-004 — FROZEN (addendum II adopted, this branch)**: **R-CANON-12** splits the data/kernel codecs (data decoder rejects capability payloads — `0x05`/standalone `0x30` ⇒ `CanonicalError::CapabilityInData`; only the kernel-mediated path produces/consumes capability payloads; `unmarshal` runs `contains_capability`, making the boundary symmetric; resolves C-14/U-02's security direction, C-78, mutation M022) and **R-PERSIST-07** makes the authority lattice durable and revalidated at recovery (snapshot carries the authority image; WAL `CapabilityGranted/Derived/Revoked`; recovery faults on dangling/generation-mismatched CapRefs; revocation monotonic across crashes; tag `RECOVERY-REVOCATION-DURABLE`, mutation M023). **SEC-022 — FROZEN (addendum III, this branch)**: R-TRUST-04 (one complete trust table; MOD-06/08/10 rows; planner never a security/runtime provider) and R-TRUST-05 (the R-DUR-02 hinge edge `ror-runtime → ror-persistence` carried; forbidden-edge mechanical check; crate-separation rule) — C-84/C-85 registered and resolved; `dep/` regeneration with SC-1/2/3 hard-gated is the recorded verification.
5. **SEC-005 — FROZEN (addendum II adopted, this branch)**: **R-MARSHAL-05** freezes the delegation surface — `Expr::Delegate` → `kernel.derive` → kernel-constructed envelope (never a plain `Value` variant), receive-side revalidation (liveness, lineage, target, generation) before registration with the recipient's `CapabilityContext` byte-identical on fault, `MarshalledValue` as checked bytes, one unified `MarshalFault` (resolves C-79 and term/ X-65 at the normative layer, mutation M024). **SEC-006 — FROZEN (addendum III, this branch)**: R-ACTOR-09 (no default spawn transfer; explicit compiler-checked manifest, strict attenuation `≺`; `trust_level` retracted; `BudgetAllocationSpec` bounded) — C-82 registered and resolved; mutation M025.
6. **SEC-007, SEC-008, SEC-017 — FROZEN (addendum IV, this branch)**: R-PLANNER-06 (exact-equality staleness, either-direction rejection, future-tagged mandatory test; C-86, M026), R-PLANNER-07 (observation channel capability-opaque: non-referential CapabilitySummary, {id, actor, digest} log shape, projection rule; C-87, M027), R-CANON-13 (15A BE sole canonical grammar, LE superseded in-source, one TAG_* namespace resolving X-50/X-54; C-92, M031).
7. **SEC-009…SEC-012 — FROZEN (addendum IV, this branch)**: R-PERSIST-08 (chained checksums, keyed iff adversarial storage, trust-table recording duty; C-88), R-RECOV-08 (reconciliation: I2 on every host path, never-re-execute, gated NotExecuted, supervisor = lifecycle not effects; C-89, M028), R-HOST-06 (durable receipts with the ResultDigest replay conjunct; C-90, M029), R-CORE-13 (closed declared fault surface, pinned resume-vs-fault, no debug text in machine values; C-91). **SEC-020 — FROZEN (addendum III, this branch)**: R-CORE-12 (fault totality: panic-free machine paths, `Fault::InternalInvariant`, transition atomicity, durable-append-first ordering, clippy denial) — C-83 registered and resolved; mutation M034.
8. **SEC-013, SEC-014, SEC-015, SEC-019, SEC-021 — FROZEN (addendum V, this branch)**: R-ARCH-05 (isolation posture decided: ladder retired, residual risk recorded, out-of-process host where host code is not fully trusted; C-93), R-CAP-10 (`AdmissibleConstraint` defined, fault-never-identity, compile-time validation; C-94, M030), R-KERN-06 (root-grant protocol with durable CapabilityGranted records, Supervisor.host issued-effect-only, planner-I/O crate separation; C-95), R-ACTOR-10 (mailbox resource admission, payload-proportional cost, footprint bounded by reserved M; C-96, M033), R-BUDGET-09 (escrow disposition totality incl. live faults and the logical-time bound; C-97, M035). **Register closed: all 23 findings frozen at the specification level (25 addendum obligations R-…, conflicts C-77…C-97 registered and resolved, mutations M019–M035, tags 2). Remaining exposure is implementation-level by construction — no implementation exists; every obligation is SPECIFIED.**
...
**Layers examined:** frozen source `Red-on-Rust.md` (grep-swept in full; ~60 sections read in depth), `spec/` (01–09 + normalization records + index), `mod/` (all 20 files), `req/` (registry parts 1–8 + method/compound/ambiguous + `registry.json` structure + `_validate.py` scope), `term/` (collisions index, laws, dictionary spot-checks), `dep/` (violations register §1–§7, graph), `README.md`. **Tooling delivered:** `audit/spec_check.py` — the SEC-023 cross-layer ID→rule consistency detector (D1 records' Original↔Normalized rule identity, D2 spec/01 body↔cited source lines, D3 spec/01 body↔spec/03 matrix rows); deterministic, stdlib-only; gate policy: D1 hard-fails (SEC-023 class), D2/D3 warn unless `--strict`; pre-restoration tree measured 154 violations (48/52/54). `audit/spec_map.py` + `audit/spec01-restoration-draft.md` — the displacement map (13 DISPLACED / 30 WEAKENED / 5 UNMATCHED) and the per-obligation restoration draft (**applied**: see §6 item 1; the draft file is retained as the review record). `audit/spec_restore_dryrun.py` — applies the draft (`--apply` for in-place; default: temporary copies) and re-measures: **48/52/54 → D1=0, with zero true-positive residuals on the restored set**. **Gate promoted:** the detector now lives at `spec/_check.py` — D1 (SEC-023 class) hard-fails; D2/D3 print `WARN [Dx]` and exit 0 unless `--strict` (the residual D2/D3 items are adjudicated terse-body/label cases on D1-clean obligations, kept visible for the owner). Current-tree run: **D1 = 0 — PASS with warnings (18×D2, 18×D3)**. `audit/spec_addendum.py` + `audit/sec001-sec002-addendum-draft.md` — the SEC-001/SEC-002 frozen-addendum applier and its generated draft: default = dry-run proof on temporary copies (checker now takes `--matrix PATH` alongside `--records`/`--spec01`), `--apply` = in-place with anchor/count safety aborts (exit 2) and an idempotency precheck whose failure on the real tree is the finding's evidence; also extends `spec/_build_index.py`'s inline dataset so index regeneration cannot silently drop the addendum. `audit/spec_addendum2.py` + `audit/spec-addendum2-draft.md` — same discipline for the five HIGH findings (SEC-003/004/005/016/018): 42 anchors, obligations 152→157, conflicts C-78…C-81 registered and resolved-by-addendum, mutations M022/M023/M024/M032, tag `RECOVERY-REVOCATION-DURABLE`; dry-run proof 157/157/148 with D1=0 and zero warnings on the five new obligations (D3 overlaps 0.76–0.92), sandboxed index rebuild 157/80/25/19; `term/` deliberately untouched (generated, source-validated register — resolutions recorded at the normative layer for its next regeneration). `audit/spec_addendum3.py` + `audit/spec-addendum3-draft.md` — the MEDIUM-HIGH freeze (SEC-006/020/022): R-ACTOR-09/R-CORE-12/R-TRUST-04/R-TRUST-05, C-82…C-85, M025/M034, and the first *full-fidelity* dry run — a `git archive` sandbox of HEAD receives the edits and the whole stack (`spec_check`, `_build_index`, `req/_validate`) runs there; that sandbox is what forced the register-count correction 81→85 (raw rows incl. the C-39 pointer) before any apply. `dep/` regeneration named as verification, register untouched (same discipline as term/). `audit/spec_addendum4.py` + `audit/spec-addendum4-draft.md` — the MEDIUM freeze (SEC-007/008/009/010/011/012/017): seven obligations (R-PLANNER-06/07, R-PERSIST-08, R-RECOV-08, R-HOST-06, R-CORE-13, R-CANON-13), C-86…C-92, M026–M029/M031; full-sandbox proof 168/168/148 with validator exit 0; the 36→35 warning delta diff-attributed to the R-RECOV-07 trailing-separator parsing artifact (relocated, not lost). `audit/spec_addendum5.py` + `audit/spec-addendum5-draft.md` — the final freeze (SEC-013/014/015/019/021): R-ARCH-05/R-CAP-10/R-KERN-06/R-ACTOR-10/R-BUDGET-09, C-93…C-97, M030/M033/M035 (registry complete M001–M035); full-sandbox proof 173/173/148, warnings identical to HEAD, validator exit 0. Adoption closes the register: 23/23 findings frozen.

...
7. **SEC-009…SEC-012** — storage authenticity decision; reconciliation protocol (incl. the declared `NotExecuted` variant); durable receipts; fault enumeration. **SEC-020 — FROZEN (addendum III, this branch)**: R-CORE-12 (fault totality: panic-free machine paths, `Fault::InternalInvariant`, transition atomicity, durable-append-first ordering, clippy denial) — C-83 registered and resolved; mutation M034.

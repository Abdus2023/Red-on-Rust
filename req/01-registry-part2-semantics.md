...
- NORMATIVE-LEVEL: MUST
- STATEMENT: `StalePlan` is a member of the fault taxonomy at the planner boundary. **Verified, correction reverted (X-64, C-54):** the statement is source-supported and stands as written — L28373 (turn [36]) states that a stale proposal “is rejected with `Fault::StalePlan`”, and this record's own SOURCE line already cited L28373. A previous revision of the terminology-normalization pass rewrote this line to say the statement was an extraction error; that rewrite was itself wrong and is reverted (R-SCOPE-03: the superseded wording is quoted, not deleted). The caveat that *does* stand is narrower: `StalePlan` is a variant of none of the seven `pub enum Fault` declarations, so it is a used-but-undeclared variant — one of twelve such `Fault::` paths (X-69) — and U-08 owns the variant set.
- PRECONDITIONS: stale proposal rejected
- POSTCONDITIONS: `Fault::StalePlan` produced. **Withdrawal reverted (X-64, C-54):** a previous revision of this pass marked this postcondition withdrawn on the claim that “the string `Fault::StalePlan` occurs nowhere in L1–42312”. That claim is false — the string occurs verbatim at L28373 — so the postcondition is restored unchanged. What remains open is that no `enum Fault` declaration contains the variant (X-69), which is U-08's decision, not a defect in this record.
- INVARIANTS: —

...
A repository-wide search of `req/*.md`, `spec/*.md` and `mod/*.md` for the line numbers of `Red-on-Rust.md` §13 (L39757-39790) and §14 (L39807-39828) returns **[('MOD-02', 'Compiler', 'frozen', 'Compiler — executable invariants (trust: Yes)'), ('MOD-03', 'Capability kernel', 'frozen', 'Capability kernel — authority decisions (trust: Yes)'), ('MOD-04', 'Budget system', 'frozen', 'Budget system — resource conservation (trust: Yes)'), ('MOD-05', 'CEK machine', 'frozen', 'CEK machine — deterministic execution (trust: Yes)'), ('MOD-06', 'no row', 'INFERRED', 'Marshalling boundary — no raw capability transfer (INFERRED: `Red-on-Rust.md` L41827-41838 has no marshalling row; the rule is R-MARSHAL-01/02, not the trust table)'), ('MOD-07', 'Scheduler', 'frozen', 'Scheduler — deterministic interleaving (trust: Yes)'), ('MOD-08', 'no row', 'INFERRED', "Effect boundary — the only path to an external effect (INFERRED: the table's 'Effect journal' row is the journal, MOD-11; the rule here is R-EFFECT-03)"), ('MOD-09', 'Replay host, Live host', 'frozen + partial', 'Replay host — recorded-effect reconstruction (trust: Yes; live host: Partial)'), ('MOD-10', 'no row', 'INFERRED', 'Canonical serializer — semantic identity (INFERRED: `Red-on-Rust.md` L41827-41838 has no serializer row; the rule is R-CANON-01/04, not the trust table)'), ('MOD-11', 'Persistence, Effect journal', 'frozen', 'Persistence / effect journal — durable + causal state (trust: Yes)'), ('MOD-12', 'Supervisor', 'frozen', 'Recovery / supervisor — lifecycle and recovery (trust: Yes)')]**. The only tracked statement of any part of it is REQ-REPO-014 (`ror-reference` … no production dependencies), whose SOURCE range L39196-40762 swallows the block without stating it. See V-02 / HD-5.
...
- Read in this document set's convention (arrow points to the dependent), L39762-39790 §13 asserts `ror-persistence` depends on `ror-runtime`, `ror-host` depends on `ror-runtime`, and `ror-agent` depends on `ror-persistence`. `spec/07` §6 states `ror-persistence -> ror-core` only and `ror-agent -> ror-core, ror-compiler, ror-runtime`. The persistence direction is a genuine contradiction, not a convention artefact: request step 14 calls `ror-persistence` append/sync from `ror-runtime` (`spec/07` §3, R-DUR-02), so the machine depends on the durable layer, and the durable layer must survive the machine. The §13 picture is a pipeline/data-flow diagram mislabelled 'Dependency Graph'.
